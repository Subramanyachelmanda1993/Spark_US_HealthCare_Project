...
do
  aws s3 --profile sparkuserprofile cp ${file} "s3://prescpipelineanalytics/dimension_city/$bucket_subdir_name/"
  echo "City File $file is pushed to S3."
done

for file in ${LOCAL_FACT_DIR}/*.*
do
  aws s3 --profile sparkuserprofile cp ${file} "s3://prescpipelineanalytics/fact/$bucket_subdir_name/"
